...
bool	is_validate_camera(t_token *token)
{
	...
	{
		err("Error\nInvalid camera orientation vector (must be in [-1,1])");
		return (false);
	}
...
bool	is_validate_light(t_token *token)
{
	if (!token->token[1] || !token->token[2] || !token->token[3])
	{
		err("Error\nLight: Missing one or more arguments");
		return (false);
	}
	if (!is_vec3_unbounded(token->token[1]))
	{
		err("Error\nLight: Invalid position vector");
		return (false);
	}
	if (!is_float_in_range(token->token[2], 0.0f, 1.0f))
	{
		err("Error\nLight: Brightness must be in [0.0, 1.0]");
		return (false);
	}
	if (!is_color_rgb(token->token[3]))
	{
		err("Error\nLight: Invalid RGB color");
		return (false);
	}
	return (true);
}
